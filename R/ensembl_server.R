...
#' \code{TRUE} on success, and \code{FALSE} otherwise. Set \code{verbose = TRUE}
...
#' @param verbose Whether to be verbose (\code{TRUE}) or not (\code{FALSE}).
#'
...
is_ensembl_reachable <-
  function(url = ensembl_server(), port = 443L, verbose = FALSE) {
